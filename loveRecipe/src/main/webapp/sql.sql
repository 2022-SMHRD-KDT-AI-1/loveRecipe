...
select * from recipe where FOOD_TYPE =  '떡,한과'  and RECIPE_COUNTRY ='한식'
                  and RECIPE_CALORY<240 ;
                  
select * from recipe
where recipe_calory<300
order by recipe_calory

-- 3. web_message테이블에 가데이터
-- num : 만들어진 시퀀스로 넣기
-- m_date : 현재 시간
insert into recipe
values(recipe_seq.nextval,'사색경단', '떡,한과', '한식', '230',
...
delete from test_refi where mb_id = 1 and ingre_name='사과';
...
insert into test_ingredient (ingre_seq , ingre_name , ingre_type ,ingre_expire ,ingre_season ,inger_temp )
values(ingre_seq.nextval, '고기','육류', '7월',50,'냉동')
...
select * from recipe where  FOOD_TYPE = '떡,한과'  and RECIPE_COUNTRY = '한식' and RECIPE_CALORY<240 
select * from recipe where  FOOD_TYPE =  '떡,한과'  and RECIPE_COUNTRY ='한식' and RECIPE_CALORY<138
select * from recipe
